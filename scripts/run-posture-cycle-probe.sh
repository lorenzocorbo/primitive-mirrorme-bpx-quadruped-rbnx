...
CONTAINER="${BPX_POSTURE_PROBE_CONTAINER:-bpx_motion_level_posture_probe}"
LABEL_KEY="ai.robonix.hardware-test"
LABEL_VALUE="mirrorme-bpx-posture-cycle"
shift || true

[[ -n "$WHEEL" && -f "$WHEEL" ]] || {
  echo "usage: $0 /path/to/bpx_sdk_open-1.0.8-cp310-...x86_64.whl --robot-ip IPV4 --confirm-physical-motion [options]" >&2
  exit 2
}
...
((confirmed)) || {
  echo "refusing physical motion: --confirm-physical-motion is required" >&2
  exit 2
}
[[ -n "$robot_ip" ]] || {
  echo "refusing physical motion: --robot-ip is required" >&2
  exit 2
}
...
bash "$PKG/scripts/verify-sdk-wheel.sh" "$WHEEL" --posture
...
echo "WARNING: this command will physically stand and sit BPX at $robot_ip"
echo "The robot must start fully lying down with a reachable physical E-stop."
...
docker run --rm --init \
  ...
  --entrypoint /bin/bash \
  "$IMAGE" \
  -lc 'python3 -m pip install --no-index --no-deps "/sdk/$BPX_SDK_WHEEL_NAME" >/dev/null && cd /pkg && exec python3 -m bpx_quadruped.posture_cycle_probe "$@"' \
  probe "$@"
